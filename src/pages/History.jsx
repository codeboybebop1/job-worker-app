...
export default function History() {
  ...
  const [filt, setFilt] = useState({ issue: {}, receive: {}, all: {} })
  ...
  const tf = filt[tab] || {}
  function setTf(key, val) { setFilt({ ...filt, [tab]: { ...filt[tab], [key]: val } }) }
  function clearTf() { setFilt({ ...filt, [tab]: {} }) }
  ...
  async function delIssue(e) { if (!confirm('Delete this issue entry?')) return; try { await removeIssue(e.id); showToast('Deleted'); ri() } catch (x) { showToast('Failed: ' + x.message) } }
  async function delReceive(e) { if (!confirm('Delete this receive entry?')) return; try { await removeReceive(e.id); showToast('Deleted'); rr() } catch (x) { showToast('Failed: ' + x.message) } }
  return (
    <div>
