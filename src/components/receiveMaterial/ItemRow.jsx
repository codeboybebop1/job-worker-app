...
export default function ItemRow({ item, iIdx, form, setForm, itemTypes, parties, fabrics, getGroups, getSizes, getParts, rmItem }) {
  ...
  return (
    <div className="section-block">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
        <div><label className="block text-[11px] font-bold text-text-soft mb-1 uppercase">Item Type</label><select className={F} value={item.itemTypeId} onChange={(e) => up({ itemTypeId: e.target.value, groupId: '', partFabric: {} })}><option value="">Select...</option>{itemTypes?.map((t) => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
